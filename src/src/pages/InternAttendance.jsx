...
import { 
  Calendar, Clock, Users, Download, FileText, TrendingUp, 
  Award, CheckCircle, XCircle, AlertCircle, UserPlus, 
  Search, Filter, ChevronLeft, ChevronRight, Edit2,
  Moon, Sun, Sparkles, GraduationCap, Coffee
} from 'lucide-react';

// ============================================================
// WellMind Brand Palette (Professional Version)
// ============================================================
...
const COLORS = {
  ...
  // UI System Colors (Cleaner Professional Look)
  bgPage: "#F8FAFC",      // Very light grey for main bg
...
/* ══════════════════════════════════════════════════════════
   TimeEditModal (Refined Professional Style)
══════════════════════════════════════════════════════════ */
      ...
      <div style={modalStyles.box} onClick={e => e.stopPropagation()}>
        ...
        <div style={modalStyles.body}>
          ...
          <div style={modalStyles.preview}>
            Preview: <b style={{ color: COLORS.actionTeal }}>{to12(timeVal)}</b>
          </div>
        </div>
...
const InternAttendance = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      collection(db, "intern_attendance", selectedDate, "records"),
      (s) => {
        const updates = {};
  ...
  const handleTimeSaved = (internId, newTime, type) => {
    setRecords((prev) =>
      prev.map((r) => r.id === internId ? { ...r, [type === 'checkin' ? 'scheduledIn' : 'scheduledOut']: newTime } : r)
    );
    ...
    try {
      ...
      pdf.autoTable({
        ...
        body: recordsRef.current.map((r, i) => {
          const s = stats[r.id] || { present: 0, late: 0, absent: 0, leave: 0 };
          return [i + 1, r.name || "—", r.role || "—", r.scheduledIn || "09:00 AM", s.present, s.late, s.absent, s.leave, s.present + s.late + s.absent + s.leave];
        }),
              ...
              {view === "daily" && (
                <div style={{display:"flex",alignItems:"center",gap:8,background:COLORS.bgPage,border:`1px solid ${COLORS.border}`,borderRadius:8,padding:4}}>
                  ...
                  <input
                    ...
                    style={{border:"none",background:"transparent",fontSize:13,fontWeight:600,color:COLORS.textMain,cursor:"pointer",outline:"none"}}
                  />
          ...
          {view === "daily" && (
            <div style={{position:"relative",marginBottom:20,maxWidth:400}}>
              ...
              <input
                ...
                style={{width:"100%",padding:"10px 12px 10px 40px",border:`1px solid ${COLORS.border}`,borderRadius:8,fontSize:14,outline:"none",background:"#fff",color:COLORS.textMain,transition:"border 0.2s"}}
...
const styles = {
  ...
  timeBadge: {
    ...
    background: COLORS.status.present.bg, color: COLORS.status.present.text, fontFamily: "monospace"
  },
  btnPrimary: {
    padding: "8px 16px",
    borderRadius: 6,
  ...
  iconBtnAction: {
    padding: "6px 12px",
    borderRadius: 6,
    border: `1px solid ${COLORS.actionTeal}`,
    background: "transparent",
  ...
  loaderContainer: {
    display: "flex", minHeight: "100vh", background: COLORS.bgPage
  },
  spinner: {
    width: 40, height: 40, border: `4px solid ${COLORS.primaryLight}`, borderTop: `4px solid ${COLORS.primary}`, borderRadius: "50%", animation: "spin 1s linear infinite"
  }
};
