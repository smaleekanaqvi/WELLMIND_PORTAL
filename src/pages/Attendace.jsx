    ...
    <div style={modalStyles.overlay} onClick={onClose}>
      <div style={modalStyles.box} onClick={e => e.stopPropagation()}>
        <div style={modalStyles.header}>
          <div style={modalStyles.avatar}>{(emp.name || "?")[0].toUpperCase()}</div>
          <div>
            <div style={modalStyles.name}>{emp.name}</div>
            <div style={modalStyles.role}>{emp.role}</div>
          </div>
        ...
        <div style={modalStyles.body}>
          ...
          <div style={modalStyles.preview}>Preview: <b style={{ color: COLORS.teal }}>{to12(timeVal)}</b></div>
        </div>
...
const Attendance = ({ onNavigate }) => {
      ...
      const results = await Promise.all(
        allDates.map(dateStr =>
          getDocs(collection(db, "attendance", dateStr, "records"))
            ...
            .catch(() => ({ dateStr, snap: { forEach: () => {} } }))
        )
      );
      results.forEach(({ dateStr, snap }) => {
        snap.forEach(d => {
          const data = d.data(), empId = d.id;
          ...
          details[empId][dateStr] = { status: data.status||'—', checkIn: data.checkIn||'—' };
  ...
  const openPersonView = async (emp) => { setSelectedEmp(emp); await fetchMonthlyReport(reportMonth); setView('person'); };
  const goToMonthly   = async ()      => { await fetchMonthlyReport(reportMonth); setView('monthly'); };
  const changeMonth   = async (dir)   => {
    const d = new Date(reportMonth + "-01");
    d.setMonth(d.getMonth() + dir);
    const nm = d.toISOString().slice(0,7);
    setReportMonth(nm);
    await fetchMonthlyReport(nm);
  };
  ...
  const downloadMonthlyPDF = async () => {
    ...
    try {
      ...
      pdf.autoTable({
        ...
        body:recordsRef.current.map((r,i)=>{ const s=stats[r.id]||{present:0,late:0,absent:0,leave:0}; return [i+1,r.name||'—',r.role||'—',s.present,s.late,s.absent,s.leave,s.present+s.late+s.absent+s.leave]; }),
        foot:[['','TOTAL','',totals.present,totals.late,totals.absent,totals.leave,totals.present+totals.late+totals.absent+totals.leave]],
        theme:'grid',
        headStyles:{fillColor:primaryRgb,textColor:[255,255,255],fontStyle:'bold',fontSize:11,halign:'center'},
        bodyStyles:{fontSize:11,halign:'center'},
        footStyles:{fillColor:primaryDarkRgb,textColor:[255,255,255],fontStyle:'bold',fontSize:11,halign:'center'},
        columnStyles:{0:{cellWidth:30},1:{halign:'left',cellWidth:140},2:{halign:'left',cellWidth:110}},
        alternateRowStyles:{fillColor:[247,245,255]}, showFoot:'lastPage',
        didParseCell(data){ if(data.section==='body'){ if(data.column.index===3){data.cell.styles.textColor=hexToRgb(COLORS.success);data.cell.styles.fontStyle='bold';} if(data.column.index===4){data.cell.styles.textColor=hexToRgb(COLORS.warning);data.cell.styles.fontStyle='bold';} if(data.column.index===5){data.cell.styles.textColor=hexToRgb(COLORS.danger);data.cell.styles.fontStyle='bold';} if(data.column.index===6){data.cell.styles.textColor=hexToRgb(COLORS.primary);data.cell.styles.fontStyle='bold';} } },
  ...
  const downloadPersonPDF = async () => {
    ...
    try {
      ...
      pdf.setFontSize(9); pdf.setTextColor(200,180,160); pdf.text(`Generated: ${new Date().toLocaleString()}`,40,70);
      const barY=100;
      ...
      pdf.autoTable({
        ...
        body:Object.entries(empDays).sort(([a],[b])=>a.localeCompare(b)).map(([ds,data],i)=>{ const d=new Date(ds); return [i+1,d.toLocaleDateString("en-US",{day:'2-digit',month:'short',year:'numeric'}),d.toLocaleDateString("en-US",{weekday:"long"}),data.checkIn||'—',data.status||'—']; }),
          ...
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
              <thead style={{background:COLORS.bgPage}}>
                <tr>
                  <th style={stylesLocal.th}>#</th>
                  <th style={{...stylesLocal.th,textAlign:"left"}}>Date</th>
                  <th style={stylesLocal.th}>Day</th>
                  <th style={stylesLocal.th}>Time</th>
              ...
              <tbody>
                {daysList.map((day,idx)=>(
                  <tr key={day.dateStr} style={{borderBottom:`1px solid ${COLORS.border}`}}>
                    <td style={{...stylesLocal.td,textAlign:"center"}}>{idx+1}</td>
                    <td style={{...stylesLocal.td,fontWeight:600}}>{day.dateLabel}</td>
                    <td style={{...stylesLocal.td,textAlign:"center"}}>{day.dayName}</td>
                    <td style={{...stylesLocal.td,textAlign:"center",fontFamily:"monospace"}}>{day.checkIn}</td>
                    <td style={{...stylesLocal.td,textAlign:"center"}}><Badge status={day.status}/></td>
                  </tr>
                ))}
              </tbody>
            </table>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",minWidth:view==="daily"?1000:900}}>
                  <thead style={{background:COLORS.bgPage,position:"sticky",top:0,zIndex:10}}>
                    {view==="daily" ? (
                      ...
                      <tr>
                        <th style={{...stylesLocal.th,textAlign:"left"}}>Employee</th>
                        <th style={{...stylesLocal.th,color:COLORS.status.present.text}}>Present</th>
                        <th style={{...stylesLocal.th,color:COLORS.status.late.text}}>Late</th>
                        <th style={{...stylesLocal.th,color:COLORS.status.absent.text}}>Absent</th>
                        <th style={{...stylesLocal.th,color:COLORS.status.leave.text}}>Leave</th>
                        <th style={stylesLocal.th}>Total</th>
                        <th style={stylesLocal.th}>Action</th>
                      </tr>
                    )}
                  </thead>
                  <tbody>
                    {records.map(r => view==="daily" ? (
                      ...
                      <tr key={r.id} style={{borderBottom:`1px solid ${COLORS.border}`}} onMouseEnter={e=>e.currentTarget.style.background=COLORS.bgPage} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                        ...
                        <td style={{...stylesLocal.td,textAlign:"center",fontWeight:700,color:COLORS.status.present.text}}>{monthlyStats[r.id]?.present||0}</td>
                        <td style={{...stylesLocal.td,textAlign:"center",fontWeight:700,color:COLORS.status.late.text}}>{monthlyStats[r.id]?.late||0}</td>
                        <td style={{...stylesLocal.td,textAlign:"center",fontWeight:700,color:COLORS.status.absent.text}}>{monthlyStats[r.id]?.absent||0}</td>
                        <td style={{...stylesLocal.td,textAlign:"center",fontWeight:700,color:COLORS.status.leave.text}}>{monthlyStats[r.id]?.leave||0}</td>
                        <td style={{...stylesLocal.td,textAlign:"center",fontWeight:700}}>{(monthlyStats[r.id]?.present||0)+(monthlyStats[r.id]?.late||0)+(monthlyStats[r.id]?.absent||0)+(monthlyStats[r.id]?.leave||0)}</td>
                        <td style={stylesLocal.td}><button onClick={()=>openPersonView(r)} style={stylesLocal.iconBtnAction}>View</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
